...
async def admin_force_return_book(borrow_id: str, admin=Depends(admin_required)):
    ...
    print(f"Admin {admin['name']} force-returned book {book['title']} for user {user_name}")
...
async def get_book_borrow_stats(book_id: int, current_user=Depends(get_current_user)):
    ...
    total_borrows = await db.borrow_records.count_documents({
        "book_id": book_id
    })
    ...
    return {
        ...
        "total_historical_borrows": total_borrows,
        "is_available": book.get("available", False)
    }